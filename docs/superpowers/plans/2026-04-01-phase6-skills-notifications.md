...
**Architecture:** Skills management reads skill metadata from disk. File watcher uses fsnotify to detect new skills generated by the dream compiler. Scope inference is a heuristic function used by the compiler prompt (documented here for reference, but executed by Claude, not Go).
...
```go
	...
	watcher, err := NewWatcher(watchDirs, func(path string) {
		...
		if err != nil {
			fmt.Printf("[AgentJIT] Compiled skill: '%s'\n", skillName)
			return
		}
		content := string(data)
		...
		fmt.Printf("[AgentJIT] Compiled skill: '%s'. Estimated savings: %s tokens/invocation.\n",
			skillName, savings)
	})
	if err != nil {
...
The `SessionStart` hook runs `agentjit daemon start --if-not-running && agentjit ingest`. The `--if-not-running` branch already outputs JSON. Update it to include the dream trigger mode:
...
echo '{"session_id":"smoke_test","hook_event_name":"PostToolUse","cwd":"/tmp","tool_name":"Bash","tool_input":{"command":"echo smoke"}}' | ./agentjit ingest
ls ~/.agentjit/logs/
cat ~/.agentjit/logs/*/smoke_test.jsonl
...
```bash
git add -A
git commit -m "feat: complete AgentJIT v0.1.0 prototype"
```
